...
#[path = "../src-tauri/vault/mod.rs"] mod vault;
#[path = "../src-tauri/git/mod.rs"] mod git;
#[path = "../src-tauri/wiki/mod.rs"] mod wiki;
#[path = "../src-tauri/search/mod.rs"] mod search;
#[path = "../src-tauri/agent/mod.rs"] mod agent;
#[path = "../src-tauri/markdown.rs"] mod markdown;
mod keys;
mod auth;
mod config;
...
mod api_tests {
    ...
    async fn post_with(app: &Router, path: &str, body: Value, cookie: Option<&str>) -> (StatusCode, HeaderMap, String) {
        ...
        let app = app.clone().layer(MockConnectInfo(SocketAddr::from(([127, 0, 0, 1], 0))));
    ...
    async fn get(app: &Router, path: &str) -> (StatusCode, HeaderMap, String) {
        let app = app.clone().layer(MockConnectInfo(SocketAddr::from(([127, 0, 0, 1], 0))));
        let resp = app.oneshot(Request::builder().method("GET").uri(path).body(Body::empty()).unwrap()).await.unwrap();
        let status = resp.status();
        let headers = resp.headers().clone();
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
        (status, headers, String::from_utf8_lossy(&bytes).to_string())
    ...
    async fn setup_admin_creates_admin_and_cookie() {
        let (app, _) = router();
        let (s, h, b) = post(&app, "/api/setup_admin", json!({"email": "a@b.c", "password": "password1"})).await;
        assert_eq!(s, StatusCode::OK, "{b}");
        let cookie = h.get(header::SET_COOKIE).and_then(|v| v.to_str().ok()).map(|c| c.to_string());
        assert!(cookie.is_some_and(|c| c.starts_with("db_session=")), "session cookie set: {h:?}");

        // second admin rejected
        let (s, _, _) = post(&app, "/api/setup_admin", json!({"email": "x@y.z", "password": "password1"})).await;
    ...
    async fn setup_admin_token_required_via_http() {
        ...
        for token in [None, Some("wrong")] {
            ...
            assert_eq!(s, StatusCode::BAD_REQUEST, "token={token:?} must be rejected");
        }
        let (_, _, b) = post(&app, "/api/setup_status", json!({})).await;
        assert_eq!(result_json(&b)["setupRequired"], true, "admin must not exist: {b}");

        // correct token → created + session cookie
        let (s, h, b) = post(&app, "/api/setup_admin", json!({"email": "a@b.c", "password": "password1", "token": "tok-secret-1"})).await;
        assert_eq!(s, StatusCode::OK, "{b}");
    ...
    async fn setup_admin_rate_limited_after_6_failures() {
        ...
        for i in 0..7 {
            ...
            let (s, _, _) = post(&app, "/api/setup_admin", json!({"email": "x", "password": "password1"})).await;
            if i < 6 {
    ...
    async fn login_flow_sets_session_cookie() {
        let (app, _) = router();
        let (s, _, _) = post(&app, "/api/setup_admin", json!({"email": "a@b.c", "password": "password1"})).await;
        assert_eq!(s, StatusCode::OK);

        // wrong password → 400
        let (s, _, _) = post(&app, "/api/login", json!({"email": "a@b.c", "password": "wrongpass"})).await;
        assert_eq!(s, StatusCode::BAD_REQUEST);

        // correct → 200 + session cookie
        let (s, h, b) = post(&app, "/api/login", json!({"email": "a@b.c", "password": "password1"})).await;
        assert_eq!(s, StatusCode::OK, "{b}");
        assert!(h.get(header::SET_COOKIE).and_then(|v| v.to_str().ok()).is_some_and(|c| c.starts_with("db_session=")));
    }
    ...
    async fn path_allowlist_blocks_escape_via_api() {
        ...
        let (s, h, _) = post(&app, "/api/setup_admin", json!({"email": "a@b.c", "password": "password1"})).await;
        assert_eq!(s, StatusCode::OK);
        ...
        let (s, _, b) = post_with(&app, "/api/open_vault", json!({"path": "/etc"}), Some(&cookie)).await;
        assert_eq!(s, StatusCode::BAD_REQUEST, "{b}");
        assert!(b.contains("outside"), "{b}");

        // traversal that resolves outside → blocked
        let escape = format!("{}/../etc", data_dir.display());
        let (s, _, _) = post_with(&app, "/api/open_vault", json!({"path": escape}), Some(&cookie)).await;
        assert_eq!(s, StatusCode::BAD_REQUEST);
        ...
        let (s, _, b) = post_with(&app, "/api/open_vault", json!({"path": format!("{}/vaults/ok", data_dir.display())}), Some(&cookie)).await;
        assert_eq!(s, StatusCode::OK, "{b}");
    }
}
