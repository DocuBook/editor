...
function VaultPicker({ onPick }: { onPick: (path: string | null) => void }) {
  ...
  return (
    <div className="fixed inset-0 flex items-start justify-center pt-[20vh] bg-black/40" onClick={() => onPick(null)}>
      <div className="w-[360px] bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-subtle)]">
          <h2 className="text-[13px] font-semibold text-[var(--text-primary)]">Open Vault</h2>
          <button onClick={() => onPick(null)} className="p-1 rounded cursor-pointer bg-transparent text-[var(--text-muted)] border-none hover:text-zinc-300"><X size={15} /></button>
        </div>

        <div className="p-3">
          {loading ? (
            <div className="flex items-center justify-center gap-2 py-6 text-xs text-[var(--text-muted)]"><Loader size={14} className="animate-spin" /> Loading vaults…</div>
          ) : vaults.length === 0 ? (
            <div className="py-4 px-3 text-xs text-[var(--text-muted)] text-center">No vaults on the server yet</div>
          ) : (
            <div className="max-h-[240px] overflow-y-auto flex flex-col gap-0.5 mb-3">
              {vaults.map(v => (
                <button key={v.path} onClick={() => onPick(v.path)} className={item + ' text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)]'}>
          ...
          <form onSubmit={create} className="flex gap-2">
            <input
              ...
              className="flex-1 min-w-0 bg-[var(--bg-primary)] border border-[var(--border)] rounded-md px-3 py-2 text-[13px] text-[var(--text-primary)] outline-none focus:border-[var(--accent)]"
            />
            <button disabled={creating || !name.trim()} className="px-3 py-2 text-xs rounded-md cursor-pointer bg-[var(--bg-hover)] text-[var(--text-primary)] border-none hover:bg-[var(--bg-tertiary)] disabled:opacity-40">
          ...
          {rootPath && (
            <button onClick={() => onPick(rootPath)} className="mt-3 w-full text-center text-[11px] text-[var(--text-muted)] hover:text-[var(--text-secondary)] cursor-pointer bg-transparent border-none">
