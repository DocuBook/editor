  ...
  return (
    <div className="flex-1 flex items-center justify-center bg-[var(--bg-primary)] p-8">
      <div className="max-w-lg w-full">
        <h2 className="text-lg font-semibold text-zinc-200 mb-1">Welcome to DocuBook</h2>
        <p className="text-sm text-zinc-500 mb-8">Your vault is ready. Here's how to get started.</p>

        <div className="space-y-5">
          {steps.map((s, i) => (
            <div key={i} className="flex gap-4">
              <div className="mt-0.5 shrink-0 w-8 h-8 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)]">
                <s.icon size={16} />
              </div>
              <div>
                <h3 className="text-sm font-medium text-zinc-300">{s.title}</h3>
        ...
        <button
          ...
          className="mt-8 px-4 py-2 bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white text-sm font-medium rounded-lg cursor-pointer border-none transition-colors"
        >
