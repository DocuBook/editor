    ...
    <aside className="ui-shell w-56 bg-surface border-r border-border-subtle flex flex-col shrink-0 h-full">
      ...
      <div className="flex items-center justify-evenly border-t border-border-subtle px-1 py-3">
        <span className="tip-wrap">
          <button onClick={openVault} className="cursor-pointer p-3 rounded-md hover:bg-surface-active text-zinc-400 hover:text-foreground transition-colors">
            <Folder size={18} />
          </button>
          <span className="tip">Open project <kbd><Command size={11} />O</kbd></span>
        </span>
        <span className="tip-wrap relative" ref={plusMenuRef}>
            <button onClick={() => setShowPlusMenu(o => !o)} data-plus-btn disabled={!isOpen} className="cursor-pointer p-3 rounded-md hover:bg-surface-active text-zinc-400 hover:text-foreground transition-colors disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent">
              <Plus size={18} />
            </button>
          ...
          {showPlusMenu && (
            <div data-plus-popup className="absolute bottom-full left-0 mb-1 bg-surface border border-border rounded-lg p-1 min-w-[180px] z-50 shadow-[0_4px_12px_rgba(0,0,0,0.3)]">
              <button onClick={() => { setShowPlusMenu(false); setCreating('file'); setNewName('') }}
                ...
                <span className="ml-auto text-[10px] text-muted font-mono flex items-center gap-0.5 whitespace-nowrap"><kbd className="inline-flex items-center gap-0.5 bg-background px-1 py-0.5 rounded-[3px] text-[10px]"><Command size={9} />N</kbd></span>
              </button>
              <button onClick={() => { setShowPlusMenu(false); setCreating('folder'); setNewName('') }}
                className="flex items-center gap-2 px-2.5 py-1.5 cursor-pointer text-[13px] text-foreground-secondary bg-transparent border-none rounded w-full text-left hover:bg-surface-active">
                <Folder size={14} /> New Folder
                <span className="ml-auto text-[10px] text-muted font-mono flex items-center gap-0.5 whitespace-nowrap"><kbd className="inline-flex items-center gap-0.5 bg-background px-1 py-0.5 rounded-[3px] text-[10px]"><Option size={9} /><Command size={9} />N</kbd></span>
              </button>
            </div>
          )}
        </span>
        <span className="tip-wrap">
          <button onClick={() => setSearchOpen(true)} disabled={!isOpen} className="cursor-pointer p-3 rounded-md hover:bg-surface-active text-zinc-400 hover:text-foreground transition-colors disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent">
            <Search size={18} />
          </button>
          <span className="tip">{isOpen ? <>Search project files <kbd><Command size={11} />F</kbd></> : 'Open a vault to search files'}</span>
        </span>
        <span className="tip-wrap">
          <button onClick={() => setSettingsOpen(true)} className="cursor-pointer p-3 rounded-md hover:bg-surface-active text-zinc-400 hover:text-foreground transition-colors">
            <Settings size={18} />
          </button>
