...
export default class ErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    logger.error('error_boundary', { error, componentStack: info.componentStack })
  }
      ...
      return (
        <div className="h-screen flex flex-col items-center justify-center gap-3 bg-background text-foreground">
          ...
          <button
            ...
            className="px-4 py-2 rounded-md bg-surface-active text-sm cursor-pointer border-none hover:bg-surface-hover"
          >
            Reload
          </button>
        </div>
