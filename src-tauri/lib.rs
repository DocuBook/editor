...
/// `vault` is behind `Arc` so blocking tasks (search, mentions) can clone the
/// handle and reuse the open vault instead of rebuilding one from its root.
